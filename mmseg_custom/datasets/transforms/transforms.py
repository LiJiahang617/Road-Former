...
import mmcv_custom
import numpy as np
from mmcv_custom.transforms.base import BaseTransform
from mmcv_custom.transforms.utils import cache_randomness
from mmengine_custom.utils import is_tuple_of
from numpy import random
from scipy.ndimage import gaussian_filter

from mmseg_custom.datasets.dataset_wrappers import MultiImageMixDataset
...
class ResizeToMultiple(BaseTransform):
    ...
    def transform(self, results: dict) -> dict:
        ...
        img = results['img']
        img = mmcv_custom.imresize_to_multiple(
            img,
            self.size_divisor,
            scale_factor=1,
        ...
        for key in results.get('seg_fields', []):
            gt_seg = results[key]
            gt_seg = mmcv_custom.imresize_to_multiple(
                gt_seg,
                self.size_divisor,
                scale_factor=1,
...
class CLAHE(BaseTransform):
    ...
    def transform(self, results: dict) -> dict:
        ...
        for i in range(results['img'].shape[2]):
            results['img'][:, :, i] = mmcv_custom.clahe(
...
class RandomRotate(BaseTransform):
    ...
    def transform(self, results: dict) -> dict:
        ...
        if rotate:
            # rotate image
            results['img'] = mmcv_custom.imrotate(
                results['img'],
                angle=degree,
            ...
            # rotate segs
            for key in results.get('seg_fields', []):
                results[key] = mmcv_custom.imrotate(
                    results[key],
                    angle=degree,
...
class AdjustGamma(BaseTransform):
    ...
    def transform(self, results: dict) -> dict:
        ...
        results['img'] = mmcv_custom.lut_transform(
    ...
    def transform(self, results: dict) -> dict:
        ...
        for key in results.get('seg_fields', []):
            if self.scale_factor != 1:
                results[key] = mmcv_custom.imrescale(
...
@TRANSFORMS.register_module()
class MultiModalPhotoMetricDistortion(BaseTransform):
    """Apply photometric distortion to image sequentially, every transformation
    is applied with a probability of 0.5. The position of random contrast is in
    second or second to last.
    ...
    def convert(self,
        ...
        # adjusted_image = Image.fromarray(img.astype(np.uint8))
        # adjusted_image.save('contrast.png')
        return img.astype(np.uint8)
    ...
    def transform(self, results: dict) -> dict:
        ...
        if c == 3:
            ...
            print(timg.shape)
            raise ValueError('the channel number should be 3, 4 or 6 for training, please check your data pipeline!')
...
class RandomRotFlip(BaseTransform):
    ...
    def random_rotate(self, results: dict) -> dict:
        ...
        results['img'] = mmcv_custom.imrotate(results['img'], angle=angle)
        for key in results.get('seg_fields', []):
            results[key] = mmcv_custom.imrotate(results[key], angle=angle)
        return results
