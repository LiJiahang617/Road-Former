...
from mmengine_custom.registry import LOOPS
...
class _InfiniteDataloaderIterator:
    ...
    recommended to use `mmengine_custom.dataset.InfiniteSampler` to enable the
    ...
    def __next__(self) -> Sequence[dict]:
        try:
            ...
            print_log(
                ...
                'recommended to use '
                '`mmengine_custom.dataset.InfiniteSampler` to enable the '
                'dataloader to iterate infinitely.',
                logger='current',
                level=logging.WARNING)
            self._epoch += 1
