...
class ModelHub:
    ...
    def get(cls, model_name):
        ...
        if metainfo is None:
            raise ValueError(
                ...
                '`mmpretrain_custom.list_models` to get all available names.')
    ...
    def _register_mmpretrain_models(cls):
        # register models in mmpretrain_custom
        if not cls.__mmpretrain_registered:
            from importlib_metadata import distribution
            root = distribution('mmpretrain_custom').locate_file('mmpretrain_custom')
...
def get_model(model: Union[str, Config],
    ...
    Examples:
        ...
        >>> import torch
        >>> from mmpretrain_custom import get_model
        ...
        >>> from mmpretrain_custom import get_model, inference_model
    ...
    from mmengine_custom.registry import DefaultScope

    from mmpretrain_custom.registry import MODELS
    with DefaultScope.overwrite_default_scope('mmpretrain_custom'):
...
def init_model(config, checkpoint=None, device=None, **kwargs):
    ...
    Args:
        config (str | :obj:`mmengine_custom.Config`): Config file path or the config
            object.
...
def list_models(pattern=None, exclude_patterns=None, task=None) -> List[str]:
    ...
    Examples:
        ...
        >>> from mmpretrain_custom import list_models
        >>> list_models()
