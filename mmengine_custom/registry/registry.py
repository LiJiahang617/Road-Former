...
class Registry:
    ...
    def infer_scope() -> str:
        ...
        if module is not None:
            ...
            # use "mmengine_custom" to handle some cases which can not infer the scope
            # like initializing Registry in interactive mode
            scope = 'mmengine_custom'
            print_log(
                'set scope as "mmengine_custom" when scope can not be inferred. You '
    ...
    def switch_scope_and_registry(self, scope: Optional[str]) -> Generator:
        ...
        Examples:
            >>> from mmengine_custom.registry import Registry, DefaultScope, MODELS
            >>> import time
            >>> # External Registry
    ...
    def build(self, cfg: dict, *args, **kwargs) -> Any:
        ...
        Examples:
            >>> from mmengine_custom import Registry
            >>> MODELS = Registry('models')
            >>> @MODELS.register_module()
            >>> class ResNet:
