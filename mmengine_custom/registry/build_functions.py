...
def build_from_cfg(
    ...
    Examples:
        >>> from mmengine_custom import Registry, build_from_cfg
    ...
    if not isinstance(registry, Registry):
        raise TypeError('registry must be a mmengine_custom.Registry object, '
...
def build_runner_from_cfg(cfg: Union[dict, ConfigDict, Config],
    ...
    Examples:
        >>> from mmengine_custom.registry import Registry, build_runner_from_cfg
    ...
    assert isinstance(
        registry, Registry), ('registry should be a mmengine_custom.Registry object',
                              f'but got {type(registry)}')
    ...
    scope = args.pop('_scope_', None)
    with registry.switch_scope_and_registry(scope) as registry:
        obj_type = args.get('runner_type', 'mmengine_custom.Runner')
        if isinstance(obj_type, str):
