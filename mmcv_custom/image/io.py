...
import numpy as np
from cv2 import (IMREAD_COLOR, IMREAD_GRAYSCALE, IMREAD_IGNORE_ORIENTATION,IMREAD_ANYDEPTH,
                 IMREAD_UNCHANGED, imdecode)
from mmengine_custom.utils import is_filepath, is_str
...
imread_flags = {
    ...
    'grayscale16': IMREAD_GRAYSCALE | IMREAD_ANYDEPTH,
...
def imread(img_or_path: Union[np.ndarray, str, Path],
    ...
    Examples:
        ...
        >>> img = mmcv_custom.imread(img_path)
        >>> img = mmcv_custom.imread(img_path, flag='color', channel_order='rgb',
        ...     backend='cv2')
        >>> img = mmcv_custom.imread(img_path, flag='color', channel_order='bgr',
        ...     backend='pillow')
        >>> s3_img_path = 's3://bucket/img.jpg'
        >>> # infer the file backend by the prefix s3
        >>> img = mmcv_custom.imread(s3_img_path)
        >>> # manually set the file backend petrel
        >>> img = mmcv_custom.imread(s3_img_path, backend_args={
        ...     'backend': 'petrel'})
        >>> http_img_path = 'http://path/to/img.jpg'
        >>> img = mmcv_custom.imread(http_img_path)
        >>> img = mmcv_custom.imread(http_img_path, backend_args={
        ...     'backend': 'http'})
    """
...
def imfrombytes(content: bytes,
    ...
    Args:
        ...
        backend (str | None): The image decoding backend type. Options are
            ...
            None, the global imread_backend specified by ``mmcv_custom.use_backend()``
    ...
    Examples:
        ...
        >>>     img_buff = f.read()
        >>> img = mmcv_custom.imfrombytes(img_buff)
        >>> img = mmcv_custom.imfrombytes(img_buff, flag='color', channel_order='rgb')
        >>> img = mmcv_custom.imfrombytes(img_buff, backend='pillow')
        >>> img = mmcv_custom.imfrombytes(img_buff, backend='cv2')
    """
...
def customfrombytes(content: bytes,
                flag: str = 'color',
                channel_order: str = 'rgb',
                backend: Optional[str] = None) -> np.ndarray:
    """Read custom dataset image from bytes. we set 3 dims images to 'rgb' channel order
    ...
    if backend == 'turbojpeg':
        ...
        img_np = np.frombuffer(content, np.uint8)
        if is_str(flag) and flag == 'grayscale16':
            flag = cv2.IMREAD_GRAYSCALE | cv2.IMREAD_ANYDEPTH
        else:
            flag = imread_flags[flag] if is_str(flag) else flag
        img = cv2.imdecode(img_np, flag) # shape of img is ori_shape at this moment
        if channel_order == 'rgb' and img.shape[2] == 3: # or may fit more than and in this situation
            cv2.cvtColor(img, cv2.COLOR_BGR2RGB, img)
        return img
...
def imwrite(img: np.ndarray,
    ...
    Examples:
        ...
        >>> ret = mmcv_custom.imwrite(img, '/path/to/img.jpg')
        >>> # infer the file backend by the prefix s3
        >>> ret = mmcv_custom.imwrite(img, 's3://bucket/img.jpg')
        >>> # manually set the file backend petrel
        >>> ret = mmcv_custom.imwrite(img, 's3://bucket/img.jpg', backend_args={
        ...     'backend': 'petrel'})
    """
    if file_client_args is not None:
        warnings.warn(
