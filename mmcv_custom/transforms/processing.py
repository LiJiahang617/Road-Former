...
import mmcv_custom
from mmcv_custom.image.geometric import _scale_size
...
class Normalize(BaseTransform):
    ...
    def transform(self, results: dict) -> dict:
        ...
        results['img'] = mmcv_custom.imnormalize(results['img'], self.mean, self.std,
                                                 self.to_rgb)
...
class Resize(BaseTransform):
    ...
    def _resize_img(self, results: dict) -> None:
        ...
        if results.get('img', None) is not None:
            if self.keep_ratio:
                img, scale_factor = mmcv_custom.imrescale(
                    results['img'],
                    results['scale'],
                ...
                # a real fix should be done in the mmcv_custom.imrescale in the future
                new_h, new_w = img.shape[:2]
                h, w = results['img'].shape[:2]
                w_scale = new_w / w
                h_scale = new_h / h
            else:
                img, w_scale, h_scale = mmcv_custom.imresize(
                    results['img'],
                    results['scale'],
                    interpolation=self.interpolation,
                    return_scale=True,
    ...
    def _resize_seg(self, results: dict) -> None:
        ...
        if results.get('gt_seg_map', None) is not None:
            if self.keep_ratio:
                gt_seg = mmcv_custom.imrescale(
                    results['gt_seg_map'],
                    results['scale'],
                    interpolation='nearest',
                    backend=self.backend)
            else:
                gt_seg = mmcv_custom.imresize(
                    results['gt_seg_map'],
                    results['scale'],
                    interpolation='nearest',
                    backend=self.backend)
...
class MultimodalResize(BaseTransform):
    """Resize images & another-modality images & bbox & seg & keypoints.
    ...
    def _resize_ano(self, results: dict) -> None:
        """Custom: Resize another modality img with ``results['scale']``."""

        if results.get('ano', None) is not None:
            if self.keep_ratio:
                ano, _ = mmcv_custom.imrescale(
                    results['ano'],
                    results['scale'],
                    interpolation=self.interpolation,
                    return_scale=True,
    ...
    def __repr__(self):
        repr_str = self.__class__.__name__
        repr_str += f' also resize another modality:'
        repr_str += f'(scale={self.scale}, '
        repr_str += f'scale_factor={self.scale_factor}, '
        repr_str += f'keep_ratio={self.keep_ratio}, '
        repr_str += f'clip_object_border={self.clip_object_border}), '
...
class Pad(BaseTransform):
    ...
    def _pad_seg(self, results: dict) -> None:
        ...
        if results.get('gt_seg_map', None) is not None:
            ...
            results['gt_seg_map'] = mmcv_custom.impad(
                results['gt_seg_map'],
...
class CenterCrop(BaseTransform):
    ...
    Args:
        ...
        pad_cfg (dict): Base config for padding. Refer to ``mmcv_custom.Pad`` for
    ...
    def _crop_img(self, results: dict, bboxes: np.ndarray) -> None:
        ...
        """
        if results.get('img', None) is not None:
            img = mmcv_custom.imcrop(results['img'], bboxes=bboxes)
    ...
    def _crop_seg_map(self, results: dict, bboxes: np.ndarray) -> None:
        ...
        """
        if results.get('gt_seg_map', None) is not None:
            img = mmcv_custom.imcrop(results['gt_seg_map'], bboxes=bboxes)
...
class MultiScaleFlipAug(BaseTransform):
    ...
    def __init__(
        ...
        if scales is not None:
            ...
            assert mmengine_custom.is_list_of(self.scales, tuple)
        else:
        ...
        assert mmengine_custom.is_list_of(self.flip_direction, str)
    ...
    def __init__(
        ...
        **resize_kwargs,
        ...
        if isinstance(scales, list):
            ...
            self.scales = [scales]
        assert mmengine_custom.is_seq_of(self.scales, (tuple, int))
...
class RandomFlip(BaseTransform):
    ...
    def __init__(self,
        ...
        if isinstance(prob, list):
            assert mmengine_custom.is_list_of(prob, float)
            assert 0 <= sum(prob) <= 1
        ...
        if isinstance(direction, str):
            ...
            assert mmengine_custom.is_list_of(direction, str)
    ...
    def _flip_seg_map(self, seg_map: dict, direction: str) -> np.ndarray:
        ...
        """
        seg_map = mmcv_custom.imflip(seg_map, direction=direction)
    ...
    def _flip(self, results: dict) -> None:
        ...
        keypoints."""
        # flip image
        results['img'] = mmcv_custom.imflip(
...
class RandomResize(BaseTransform):
    ...
    def _random_sample(scales: Sequence[Tuple[int, int]]) -> tuple:
        ...
        assert mmengine_custom.is_list_of(scales, tuple) and len(scales) == 2
