...
import torch
from mmcv_custom.ops.roi_align import roi_align
...
class BaseInstanceMasks(metaclass=ABCMeta):
    ...
    def rescale(self, scale, interpolation='nearest'):
        ...
        For details can refer to `mmcv_custom.imrescale`.

        Args:
            scale (tuple[int]): The maximum size (h, w) of rescaled mask.
            interpolation (str): Same as :func:`mmcv_custom.imrescale`.
    ...
    def resize(self, out_shape, interpolation='nearest'):
        ...
        Args:
            ...
            interpolation (str): See :func:`mmcv_custom.imresize`.
    ...
    def crop_and_resize(self,
        ...
        Args:
            ...
            interpolation (str): See `mmcv_custom.imresize`
    ...
    def translate(self,
        ...
        Args:
            ...
            interpolation (str): Same as :func:`mmcv_custom.imtranslate`.
    ...
    def shear(self,
        ...
        Args:
            ...
            interpolation (str): Same as in :func:`mmcv_custom.imshear`.
    ...
    def get_bboxes(self, dst_type='hbb'):
        ...
        Please refer to ``mmdet_custom.structures.bbox.box_type`` for more details of
        the box type.
...
class BitmapMasks(BaseInstanceMasks):
    ...
    Example:
        >>> from mmdet_custom.data_elements.mask.structures import *  # NOQA
    ...
    def rescale(self, scale, interpolation='nearest'):
        ...
        if len(self.masks) == 0:
            new_w, new_h = mmcv_custom.rescale_size((self.width, self.height), scale)
            rescaled_masks = np.empty((0, new_h, new_w), dtype=np.uint8)
        else:
            rescaled_masks = np.stack([
                mmcv_custom.imrescale(mask, scale, interpolation=interpolation)
    ...
    def flip(self, flip_direction='horizontal'):
        ...
        if len(self.masks) == 0:
            ...
            flipped_masks = np.stack([
                mmcv_custom.imflip(mask, direction=flip_direction)
                for mask in self.masks
            ])
    ...
    def pad(self, out_shape, pad_val=0):
        ...
        if len(self.masks) == 0:
            ...
            padded_masks = np.stack([
                mmcv_custom.impad(mask, shape=out_shape, pad_val=pad_val)
                for mask in self.masks
            ])
    ...
    def translate(self,
        ...
        Example:
            >>> from mmdet_custom.data_elements.mask.structures import BitmapMasks
        ...
        if len(self.masks) == 0:
            ...
            if masks.shape[-2:] != out_shape:
                ...
                masks = empty_masks
            translated_masks = mmcv_custom.imtranslate(
    ...
    def shear(self,
        ...
        if len(self.masks) == 0:
            ...
            sheared_masks = mmcv_custom.imshear(
    ...
    def rotate(self,
        ...
        Args:
            ...
            scale (int | float): Isotropic scale factor.
            border_value (int | float): Border value. Default 0 for masks.
            interpolation (str): Same as in :func:`mmcv_custom.imrotate`.
        ...
        if len(self.masks) == 0:
            ...
            rotated_masks = mmcv_custom.imrotate(
                self.masks.transpose((1, 2, 0)),
                angle,
                center=center,
                scale=scale,
    ...
    def random(cls,
        ...
        """
        from mmdet_custom.utils.util_random import ensure_rng
        rng = ensure_rng(rng)
...
class PolygonMasks(BaseInstanceMasks):
    ...
    def random(cls,
        ...
        Example:
            >>> from mmdet_custom.data_elements.mask.structures import PolygonMasks
