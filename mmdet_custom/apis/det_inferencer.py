...
from mmengine_custom.fileio import (get_file_backend, isdir, join_path,
                                    list_dir_or_file)
from mmengine_custom.infer.infer import BaseInferencer, ModelType
from mmengine_custom.model.utils import revert_sync_batchnorm
from mmengine_custom.registry import init_default_scope
from mmengine_custom.runner.checkpoint import _load_checkpoint_to_model
from mmengine_custom.visualization import Visualizer
from rich.progress import track

from mmdet_custom.evaluation import INSTANCE_OFFSET
from mmdet_custom.registry import DATASETS
from mmdet_custom.structures import DetDataSample
from mmdet_custom.structures.mask import encode_mask_results, mask2bbox
from mmdet_custom.utils import ConfigType
...
class DetInferencer(BaseInferencer):
    ...
    Args:
        ...
        scope (str, optional): The scope of the model. Defaults to mmdet_custom.
    ...
    def __init__(self,
                 ...
                 scope: Optional[str] = 'mmdet_custom',
    ...
    def _load_weights_to_model(self, model: nn.Module,
        ...
        if checkpoint is not None:
            ...
            if 'dataset_meta' in checkpoint_meta:
                # mmdet_custom 3.x, all keys should be lowercase
                model.dataset_meta = {
                    k.lower(): v
    ...
    def _init_pipeline(self, cfg: ConfigType) -> Compose:
        ...
        pipeline_cfg[load_img_idx]['type'] = 'mmdet_custom.InferencerLoader'
    ...
    def visualize(self,
        ...
        for single_input, pred in zip(inputs, preds):
            if isinstance(single_input, str):
                img_bytes = mmengine_custom.fileio.get(single_input)
                img = mmcv_custom.imfrombytes(img_bytes)
                img = img[:, :, ::-1]
    ...
    def pred2dict(self,
        ...
        if 'pred_panoptic_seg' in data_sample:
            ...
            if is_save_pred:
                mmcv_custom.imwrite(pan[:, :, ::-1], out_img_path)
                result['panoptic_seg_path'] = out_img_path
            else:
                result['panoptic_seg'] = pan

        if is_save_pred:
            mmengine_custom.dump(result, out_json_path)
