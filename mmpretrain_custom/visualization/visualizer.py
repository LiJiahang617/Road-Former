...
from mmengine_custom.dataset import BaseDataset
from mmengine_custom.dist import master_only
from mmengine_custom.visualization import Visualizer
from mmengine_custom.visualization.utils import img_from_canvas

from mmpretrain_custom.registry import VISUALIZERS
from mmpretrain_custom.structures import DataSample
...
class UniversalVisualizer(Visualizer):
    ...
    def visualize_cls(self,
        ...
        Args:
            ...
            text_cfg (dict): Extra text setting, which accepts
                arguments of :meth:`mmengine_custom.Visualizer.draw_texts`.
        ...
        if resize is not None:
            h, w = image.shape[:2]
            if w < h:
                image = mmcv_custom.imresize(image, (resize, resize * h // w))
            else:
                image = mmcv_custom.imresize(image, (resize * w // h, resize))
        elif rescale_factor is not None:
            image = mmcv_custom.imrescale(image, rescale_factor)
        ...
        if out_file is not None:
            ...
            mmcv_custom.imwrite(drawn_img[..., ::-1], out_file)
        else:
    ...
    def visualize_image_retrieval(self,
        ...
        if resize is not None:
            image = mmcv_custom.imrescale(image, (resize, resize))
        ...
        for k, (score, sample_idx) in enumerate(zip(match_scores, indices)):
            ...
            value_image = mmcv_custom.imread(sample['img_path'])[..., ::-1]
    ...
    def add_mask_to_image(
        ...
        image = mmcv_custom.imresize(image, resize)
        self.set_image(image)
    ...
    def visualize_visual_grounding(self,
        ...
        if resize is not None:
            h, w = image.shape[:2]
            if w < h:
                image, w_scale, h_scale = mmcv_custom.imresize(
