...
import numpy as np
import torch
from mmcv_custom.image import imread
from mmengine_custom.config import Config
from mmengine_custom.dataset import BaseDataset, Compose, default_collate

from mmpretrain_custom.registry import TRANSFORMS
from mmpretrain_custom.structures import DataSample
from mmpretrain_custom.utils import track
...
class TextToImageRetrievalInferencer(BaseInferencer):
    ...
    Example:
        >>> from mmpretrain_custom import TextToImageRetrievalInferencer
    ...
    def _prepare_prototype(self, prototype, cache=None, batch_size=8):
        from mmengine_custom.dataset import DefaultSampler
        ...
        if isinstance(prototype, list):
            ...
            # A config of dataset
            from mmpretrain_custom.registry import DATASETS
            test_pipeline = [dict(type='LoadImageFromFile'), self.img_pipeline]
    ...
    def visualize(self,
        ...
        if self.visualizer is None:
            from mmpretrain_custom.visualization import UniversalVisualizer
...
class ImageToTextRetrievalInferencer(BaseInferencer):
    ...
    Example:
        >>> from mmpretrain_custom import ImageToTextRetrievalInferencer
    ...
    def _prepare_prototype(self, prototype, cache=None, batch_size=8):
        ...
        if isinstance(prototype, str):
            ...
            dataset = mmengine_custom.list_from_file(prototype)
        elif mmengine_custom.utils.is_seq_of(prototype, str):
            dataset = prototype
        else:
            raise TypeError(f'Unsupported prototype type {type(prototype)}.')
