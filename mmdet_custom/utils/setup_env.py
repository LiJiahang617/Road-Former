...
def register_all_modules(init_default_scope: bool = True) -> None:
    """Register all modules in mmdet_custom into the registries.

    Args:
        init_default_scope (bool): Whether initialize the mmdet_custom default scope.
            When `init_default_scope=True`, the global default scope will be
            set to `mmdet_custom`, and all registries will build modules from mmdet_custom's
            registry node. To understand more about the registry, please refer
            to https://github.com/open-mmlab/mmengine/blob/main/docs/en/tutorials/registry.md
            Defaults to True.
    """  # noqa
    import mmdet_custom.datasets  # noqa: F401,F403
    import mmdet_custom.engine  # noqa: F401,F403
    import mmdet_custom.evaluation  # noqa: F401,F403
    import mmdet_custom.models  # noqa: F401,F403
    import mmdet_custom.visualization  # noqa: F401,F403

    if init_default_scope:
        never_created = DefaultScope.get_current_instance() is None \
                        or not DefaultScope.check_instance_created('mmdet_custom')
        if never_created:
            DefaultScope.get_instance('mmdet_custom', scope_name='mmdet_custom')
            return
        current_scope = DefaultScope.get_current_instance()
        if current_scope.scope_name != 'mmdet_custom':
            warnings.warn('The current default scope '
                          f'"{current_scope.scope_name}" is not "mmdet_custom", '
                          '`register_all_modules` will force the current'
                          'default scope to be "mmdet_custom". If this is not '
                          'expected, please set `init_default_scope=False`.')
            # avoid name conflict
            new_instance_name = f'mmdet_custom-{datetime.datetime.now()}'
            DefaultScope.get_instance(new_instance_name, scope_name='mmdet_custom')
