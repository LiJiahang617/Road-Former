...
class ModulatedDeformConv2dFunction(Function):
    ...
    def symbolic(g, input, offset, mask, weight, bias, stride, padding,
        ...
        return g.op(
            'mmcv_custom::MMCVModulatedDeformConv2d',
            *input_tensors,
            stride_i=stride,
            padding_i=padding,
            dilation_i=dilation,
...
@MODELS.register_module('DCNv2Mask')
class ModulatedDeformConv2dMask(ModulatedDeformConv2d):
    """A ModulatedDeformable Conv Encapsulation that acts as normal Conv
    layers.
    ...
    def _load_from_state_dict(self, state_dict, prefix, local_metadata, strict,
        ...
        if version is not None and version > 1:
            print_log(
                f'ModulatedDeformConvMask {prefix.rstrip(".")} is upgraded to '
                'version 2.',
                logger='current')
